...
private:
    ...
    //static const short SCREEN_DN_STOP_THRESHOLD = 48;
...
public:
    ...
    Screen() : error_(false), motor_dir_(DIR_NEUTRAL), pos_(0) {
        /* nop */
    }
...
ISR(ANA_COMP_vect) {
    static bool last = 0;
    bool got_falling_edge = (ACSR & (1<<ACO)) == 0;
...
int main(void) {
    ...
    for (;;) {
        ...
        if (screen.error()) { // blink LED.
            status_led(Clock::now() & 2048);
        } else {
